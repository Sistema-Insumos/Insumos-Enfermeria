...
export function SubjectDetailPage() {
  ...
  return (
    <div>
      ...
      {modalOpen && subjectId && (
        <NewWorkshopModal subjectId={subjectId} onClose={() => setModalOpen(false)} />
      )}
    </div>
  );
}

function NewWorkshopModal({ subjectId, onClose }: { subjectId: string; onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-md rounded-lg bg-surface-lowest p-6 shadow-xl">
        <div className="mb-4 flex items-start justify-between">
          <h2 className="text-xl font-bold text-on-surface">Nuevo Taller</h2>
        ...
        >
          ...
          {mutation.isError && <p className="text-sm text-danger">No se pudo crear el taller.</p>}
