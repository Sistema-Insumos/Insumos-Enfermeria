  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-on-surface">Equipamiento General</h1>
          <p className="mt-1 text-on-surface-variant">
            Todos los equipos registrados, en cualquier sala.
          </p>
        </div>
        <button
          ...
          <tbody>
            {items.map((eq) => (
              <tr key={eq.id} className="border-t border-outline-variant hover:bg-surface-low">
                ...
                <td className="px-4 py-3">
                  <Link
                    to={`/equipamiento/${eq.roomId}`}
                    className="rounded-full bg-surface-container px-2 py-0.5 text-xs font-semibold text-secondary hover:underline"
                  >
                    {eq.room?.name ?? "—"}
                  </Link>
                </td>
      ...
      {modalOpen && (
        <EquipmentFormModal rooms={roomsQuery.data ?? []} onClose={() => setModalOpen(false)} />
      )}
      {editingEquipment && (
        <EquipmentFormModal
